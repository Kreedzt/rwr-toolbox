...
2. `docs/UI.md` - 设计系统 token、字号与弱化阶梯、共享组件目录、800×600 约束
3. `docs/PLAN.md` - 实现参考（Ping/解析/Hotkeys/Mods 等）
4. `docs/CONSTRUCTION.md` - 项目宪法：管辖全部开发的七条核心原则
...
| `CONSTRUCTION.md` | 核心原则变更时 | 七条不可协商原则与其理由 |
...
本项目遵循 [`docs/CONSTRUCTION.md`](./CONSTRUCTION.md) 中定义的七大核心原则：

1. **Desktop-First UI Design** - 800×600 最小分辨率，高信息密度
2. **Internationalization (i18n)** - Transloco 运行时国际化
3. **Theme Adaptability** - 自定义军事风 `light`/`dark` 双主题是唯一色彩事实源
4. **Signal-Based State Management** - Angular v20 Signals 模式，不用 `BehaviorSubject`／`toSignal()` 桥接
5. **Documentation-Driven Development** - `docs/` 为单一真实来源
6. **Icon Management** - lucide-angular 集中式注册
7. **Tailwind-First Styling** - 共享组件 → Tailwind utility → DaisyUI；自定义 CSS 是最后手段
